...
def execution(G, algorithm, split, iteration, edge_group, percentual_to_time):
    G_disturbed, train, test, hidden = disturbed_hin(G, split=split, random_state=(1 + iteration),
    ...
    if algorithm == 'deep_walk':
        for key, value in percentual_to_time.items():
            ...
            restored_df.to_csv(
                "{}results/{}_{}_{}_{}_{}_{}.csv".format(path, file_name, algorithm, split, edge_group, iteration, key),
                index=False)
            G_found, hidden, train, test = true_restore(G_found, hidden, train, test, percentual=value,
                                                        edge_group=edge_group)
