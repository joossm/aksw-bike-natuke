...
def execution(algorithm, split, iteration, edge_group, evaluation_stages):
    ...
    with open(f"{path}/hin-final.gpickle", "rb") as fh:
        G = pickle.load(fh)
    ...
    if algorithm == 'deep_walk':
        for evaluation_stage in evaluation_stages:
            ...
            with open("{}results/execution_time.txt".format(path), 'a') as f:
                f.write(
                    f'{algorithm},{split},{iteration},{edge_group},{evaluation_stage},{(time.time() - start_time)}\n')
            restored_df.to_csv(
                "{}results/{}_{}_{}_{}_{}_{}.csv".format(path, file_name, algorithm, split, edge_group, iteration,
                                                         evaluation_stage), index=False)
