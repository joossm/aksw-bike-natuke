...
def disturbed_hin(G, split=0.6, random_state=None, extra_cut_from='nubbe', edge_group='doi_bioActivity',
                  node_from_feature='node_from', type_feature='edge_group', group_feature='group'):
    """
    G: hin;
    ...
    for _, row in edges_group.iterrows():
        if row['type'] == edge_group:
            train[row['type']] = edges[edges['type'] == row['type']].reset_index(drop=True).loc[
                                 row['to_cut_count']:].reset_index(drop=True)
            test[row['type']] = edges[edges['type'] == row['type']].reset_index(drop=True).loc[
                                :row['to_cut_count'] - 1].reset_index(drop=True)
        ...
        for _, row in tc_df.iterrows():
            ...
            for neighbor in neighbors_list:
                if G_disturbed.nodes[neighbor][node_from_feature] == extra_cut_from:
                    has_cut = True
                    neighbors_hidden.append(
                        {'neighbor': neighbor, 'edge_group': G_disturbed[row['node']][neighbor][type_feature]})
...
def run_knn(k, G_restored, row, knn_data, knn_nodes, node_feature='node', embedding_feature='f'):
    ...
    knn.fit(knn_data)
    indice = knn.kneighbors(G_restored.nodes[row[node_feature]][embedding_feature].reshape(-1, 512),
...
def restore_hin(G, cutted_dict, n_jobs=-1, k=-1, node_feature='node', neighbor_feature='neighbor',
    ...
    def process(start, end, G, key, value, return_dict, thread_id):
        ...
        for _, row in tqdm(value_thread.iterrows(), total=value_thread.shape[0]):
            ...
            edge_to_add = [row[node_feature] if e == G.nodes[row[node_feature]][group_feature] and row[node_feature] !=
                                                edge_to_add[0] else e for e in edge_to_add]
...
def ml_restore_hin(G, train, test, edge_group='doi_bioActivity', neighbor_feature='neighbor', node_feature='node',
                   embedding_feature='f', min_delta=0.00001, patience=10, epochs=1000, embedding_size=512):
    ...
    def getY(train, test, neighbor_feature='neighbor'):
        classes = pd.Series(
            train[edge_group][neighbor_feature].to_list() + test[edge_group][neighbor_feature].to_list()).unique()
        classes_codes = {}
    ...
    X_train, X_test = getX(G, train, test, edge_group=edge_group, node_feature=node_feature,
...
def true_restore(G, hidden, train, test, percentual=1.0, edge_group='doi_bioActivity', node_feature='node',
                 neighbor_group_feature='neighbor_group', neighbor_feature='neighbor', edge_group_feature='edge_group'):
    G_found = deepcopy(G)
...
def metapath2vec(graph, dimensions=512, num_walks=1, walk_length=100, context_window_size=10,
                 ...
                 user_metapaths=[
                     ['doi', 'name', 'doi'], ['doi', 'bioActivity', 'doi'], ['doi', 'molType', 'doi'],
                     ['doi', 'collectionSpecie', 'doi'],
                     ['doi', 'collectionSite', 'doi'], ['doi', 'collectionType', 'doi'],
                     ['doi', 'molecularMass', 'doi'],
                     ['doi', 'monoisotropicMass', 'doi'], ['doi', 'cLogP', 'doi'], ['doi', 'tpsa', 'doi'],
                     ['doi', 'numberOfLipinskiViolations', 'doi'], ['doi', 'numberOfH_bondAcceptors', 'doi'],
                     ['doi', 'numberOfH_bondDonors', 'doi'],
